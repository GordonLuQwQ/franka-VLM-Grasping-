  ...
  franka_msgs__msg__FrankaRobotState__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    franka_msgs__msg__FrankaRobotState * data =
      (franka_msgs__msg__FrankaRobotState *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!franka_msgs__msg__FrankaRobotState__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          franka_msgs__msg__FrankaRobotState__fini(&data[i]);
        }
        free(data);
        return false;
      }
